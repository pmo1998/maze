...
class Node extends Component {
    ...
    onMouseEnter=(e)=>{
      if(isStartSelected&&!this.state.isFinish) this.setState({color:this.getColor('red')});
      if(isFinishSelected&&!this.state.isStart) this.setState({color:this.getColor('green')});
      if(isFreeSelected&&!this.state.isFinish&&!this.state.isStart) this.changeWalkable.call(this);
    }

    onMouseLeave=(e)=>{
      if((isStartSelected&&!this.state.isFinish || isFinishSelected&&!this.state.isStart)) this.setState({color:this.getColor('black')});
  }
